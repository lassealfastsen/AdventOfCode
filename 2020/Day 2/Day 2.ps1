...
#Part 1
Function Test-PasswordValidity {
    Param(
        [Parameter(Mandatory=$true, Position=1)][int]$minCharOccurences,
        [Parameter(Mandatory=$true, Position=2)][int]$maxCharOccurences,
        [Parameter(Mandatory=$true, Position=3)][char]$character,
        [Parameter(Mandatory=$true, Position=4)][string]$password
    )

    $charOccurences = ($password.ToCharArray() | Where-Object {($_ -eq $character)} | Measure-Object).Count
    if (($charOccurences -ge $minCharOccurences) -and ($charOccurences -le $maxCharOccurences)) {
        return $true
    } else {
        return $false
    }
...
Function Test-PasswordValidity {
    Param(
        [Parameter(Mandatory=$true, Position=1)][int]$firstPosition,
        [Parameter(Mandatory=$true, Position=2)][int]$secondPosition,
        [Parameter(Mandatory=$true, Position=3)][char]$character,
        [Parameter(Mandatory=$true, Position=4)][string]$password
    )
    ...
    if (($charArray[$firstPosition-1] -eq $char) -ne ($charArray[$secondPosition-1] -eq $char)) {
        return $true
    } else {
        return $false
    }
...
$validPasswords = 0
foreach ($password in $inputpasswords) {
    $restrictions = $password.Split(" ")[0]
    $min = $restrictions.split("-")[0]
    $max = $restrictions.split("-")[1]
    $char = $password.Split(" ")[1].Replace(":", "")
