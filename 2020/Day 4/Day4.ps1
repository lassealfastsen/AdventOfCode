...
#Part 1
Function Test-PassportValitidy {
    Param(
        [Parameter(Mandatory=$true)][Passport]$passport
    )

    return (
        ($passport.BirthYear -ne 0) `
        -and ($passport.IssueYear -ne 0) `
        -and ($passport.ExpirationYear -ne 0) `
        -and ($passport.Height -notlike '') `
        -and ($passport.HairColor -notlike '') `
        -and ($passport.EyeColor -notlike '') `
        -and ($passport.PassportID -notlike '') `
...
Function Test-PassportValitidy {
    ...
    return (
        (
            ($passport.BirthYear -ne 0) `
            -and (($passport.BirthYear -ge 1920) -and ($passport.BirthYear -le 2002)) #1920 <= BirthYear >= 2002
        ) `
        -and (
            ($passport.IssueYear -ne 0) `
            -and (($passport.IssueYear -ge 2010) -and ($passport.IssueYear -le 2020)) #2010 <= IssueYear >= 2020
        ) `
        -and (
            ($passport.ExpirationYear -ne 0) `
            -and (($passport.ExpirationYear -ge 2020) -and ($passport.ExpirationYear -le 2030)) #2020 <= ExpirationYear >= 2030
        ) `
        -and (
            $passport.Height -notlike '' `
            -and (
                (
                    ($passport.Height -match '([0-9])*([c][m])')`
                    -and (($passport.Height.Replace('cm', '') -ge 150) -and ($passport.Height.Replace('cm', '') -le 193))
                ) `
                -or (
                    ($passport.Height -match '([0-9])*([i][n])')`
                    -and (($passport.Height.Replace('in', '') -ge 59) -and ($passport.Height.Replace('in', '') -le 76))
                )
            ) `
        ) `
        -and (
            ($passport.HairColor -notlike '') `
            -and ($passport.HairColor -match '^#(?:[0-9a-fA-F]{3}){1,2}$')
        ) `
        -and (
            ($passport.EyeColor -notlike '') `
            -and ($passport.EyeColor -in @('amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'))
        ) `
        -and (
            ($passport.PassportID -notlike '') `
            -and ($passport.PassportID -match '^([0-9]){9}$')
        )
    )
}
...
[Passport[]]$ArrPassports = @()
$objPassport = New-Object -TypeName Passport
ForEach ($line in $($passportDB -split "`r`n"))
{
    if ($line -eq '') {
        $ArrPassports += $ObjPassport
        $objPassport = New-Object -TypeName Passport
        continue
    }

    $arrValues = $line.Split(' ')
    foreach ($value in $arrValues) {
        $key = $value.Split(':')[0]
        $val = $value.Split(':')[1]
        
        switch ($key) {
            'byr' { $objPassport.BirthYear = $val }
            'iyr' { $objPassport.IssueYear = $val }
            'eyr' { $objPassport.ExpirationYear = $val }
            'hgt' { $objPassport.Height = $val }
            'hcl' { $objPassport.HairColor = $val }
            'ecl' { $objPassport.EyeColor = $val }
            'pid' { $objPassport.PassportID = $val }
            'cid' { $objPassport.CountryID = $val }
            Default { Write-HOst "Error in line $line value '$value'"}
        }
    }
}
...
$validPassports = 0
foreach ($pass in $ArrPassports) {
    if (Test-PassportValitidy $pass) {$validPassports++}
}
